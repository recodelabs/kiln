...
fn cells(resource: &Value) -> Vec<(String, u64, String)> {
    resource["extension"]
        ...
        .filter(|e| e["url"] == EXT)
        .map(|e| {
            let sub = |name: &str| e["extension"].as_array().unwrap().iter().find(|x| x["url"] == name).unwrap().clone();
            (
...
fn index_writes_only_the_positioned_locations_that_lack_a_requested_cell() {
    ...
    write_snapshot(
        ...
        &[
            ...
            loc("has-cell", Some((9.9, 10.5)), vec![cell_ext("quadkey", 18, "123123123123123123")]),
            loc("other-level", Some((9.9, 10.5)), vec![cell_ext("quadkey", 10, "1231231231")]),
    ...
    assert!(stdout.contains("1 already indexed, 1 without a position"), "{stdout}");
    ...
    assert_eq!(needs["meta"]["versionId"], "7", "version kept for the if-match PUT");
    assert_eq!(needs["name"], "needs-cell", "rest of the resource untouched");
    let c = cells(needs);
    assert_eq!(c.len(), 1);
    ...
    let c = cells(other);
    assert_eq!(c.len(), 2, "the zoom-10 cell is kept, the zoom-18 one added");
...
fn refresh_recomputes_a_present_cell_and_a_bad_scheme_is_a_usage_error() {
    ...
    write_snapshot(
        dir.path(),
        &[loc("stale", Some((9.9, 10.5)), vec![cell_ext("quadkey", 18, "000000000000000000")])],
    );
    let out = dir.path().join("changes.ndjson");
    ...
    let fixed: Value = serde_json::from_str(std::fs::read_to_string(&out).unwrap().lines().next().unwrap()).unwrap();
